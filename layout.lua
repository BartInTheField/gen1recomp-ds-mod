...
function Layout.surfaceSize(w, h)
  w = w or Layout.W
  h = h or Layout.H
  return w, h * 2 + Layout.GAP
end

-- regions in framebuffer pixels: the integer scale plus the top (world) and
-- bottom (ui) boxes.  pw/ph are the framebuffer pixel size of the window.
function Layout.regions(pw, ph, w, h)
  w = w or Layout.W
  h = h or Layout.H
  local surfW, surfH = Layout.surfaceSize(w, h)
...
function Layout.screenAt(px, py, pw, ph, w, h)
  local _, world, ui = Layout.regions(pw, ph, w, h)
